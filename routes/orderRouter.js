...
const { OrderItem, validateOrderItem } = require('../models/order-item')
  ...
  if (error) return res.status(400).send(error.details[0].message);
    ...
    const orderItemsIdsResolved = await Promise.all(
        req.body.orderItems.map(async (orderItem) => {
            ...
            /*
            let { error } = validateOrderItem(orderItem);
            if (error) {
...
router.get(`/get/userorders/:userid`, validateObjectId, async (req, res) => {
